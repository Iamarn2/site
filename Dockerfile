...
ENV DJANGO_SETTINGS_MODULE=slides.settings.production DJANGO_DEBUG=off

# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=slides/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/slides/media/"
...
RUN mkdir -p /code/slides/media/images && chown -R 1000:2000 /code/slides/media

# mark the destination for images as a volume
VOLUME ["/code/slides/media/images/"]
