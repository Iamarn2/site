...
from slides.base.models import FooterText
...
def get_site_root(context):
    ...
    # Главное меню должно иметь site.root_page
...
def has_children(page):
    ...
    # Разрешать индексным страницам перечислять свои дочерниt элементы
    return page.get_children().live().exists()


def is_active(page, current_page):
    # To give us active state on main navigation
    # Активное состояние в главной навигации
...
# Извлекает верхние пункты меню — непосредственные дочерние элементы родительской страницы.
# Метод has_menu_children необходим, потому что меню Foundation требует
# выпадающий класс, который будет применен к родителю
...
def top_menu(context, parent, calling_page=None):
    ...
    return {
        ...
        # Тег pageurl, который мы хотим использовать в этом шаблоне
        "request": context["request"],
    }


# Retrieves the children of the top menu items for the drop downs
# Извлекает дочерние элементы верхних элементов меню для раскрывающихся списков.
...
# Получить текст нижнего колонтитула из контекста
