...
from slides.base.models import FooterText, HomePage, Person, StandardPage
from slides.blog.models import BlogIndexPage, BlogPage
from slides.breads.models import (
    BreadIngredient,
    BreadPage,
    BreadsIndexPage,
    BreadType,
...
from slides.locations.models import LocationPage, LocationsIndexPage

FIXTURE_MEDIA_DIR = Path(settings.PROJECT_DIR) / "base/fixtures/media/original_images"


class Command(BaseCommand):
    help = "Создает случайные данные. Полезно для тестирования производительности или нагрузки."

    def add_arguments(self, parser):
        parser.add_argument(
            "page_count",
            type=int,
            help="Сколько страниц каждого типа создать",
        )
        parser.add_argument(
            "snippet_count",
            type=int,
            help="Сколько сниппетов каждого типа создать",
        )
        parser.add_argument(
            "image_count",
            type=int,
            help="Сколько изображений создать",
        )
